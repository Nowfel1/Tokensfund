...
// We re-quote on the server at swap time so the deposit address is bound to a
// fresh, executable quote — never trust a stale client-supplied quote.
export async function POST(req: NextRequest) {
  try {
    ...
    if (!body.destinationAddress) {
      return NextResponse.json({ error: "A destination address is required to open a deposit address." }, { status: 400 });
    }
    ...
    if (body.provider === "chainflip") {
      ...
      return NextResponse.json(await chainflip.buildSwap(quote, body));
    }
    ...
    return NextResponse.json({ error: e.message ?? "Could not open deposit address" }, { status: 500 });
  }
}
